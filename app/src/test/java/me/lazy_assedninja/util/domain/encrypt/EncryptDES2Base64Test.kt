...
class EncryptDES2Base64Test {
    ...
    private lateinit var encryptDES2Base64: EncryptDES2Base64

    @Before
    fun init() {
        encryptDES2Base64 = EncryptDES2Base64(mainCoroutineRule.dispatcher, encrypt)
    }

    @Test
    fun invoke() = runTest {
        `when`(encrypt.encryptDES2Base64(data, key, transformation, iv))
            .thenReturn(result)

        val loaded = encryptDES2Base64.invoke(data, key, transformation, iv)
        verify(encrypt).encryptDES2Base64(data, key, transformation, iv)
